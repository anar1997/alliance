...
import { useState } from "react";
import YesorNoInterruption from "./YesorNoInterruption";
// import { set } from "date-fns";



function EditInterruption({empoyeeid, user, setOpenInterruption, openInterruption}) {
  ...
  const cony = `https://dev.kodaze.com/api/v1/salaries/salary-deduction/?employee=${employeeid}&salary_date__month=${[month]}&salary_date__year=${[yearsClick]}`;
  useEffect(() => {
    ...
    const deleteInterruption = come.map(m => m.is_paid === false && m.id) 
...
//     const addlink = `https://dev.kodaze.com/api/v1/salaries/salary-deduction/${putreq}/`;
...
//         },
//       }
//         ).then(alert("success")).then(setOpenInterruption(!openInterruption))
//   }
  ...
  return (
    <FullScreen className="h-full">
     {yesorNo ? <YesorNoInterruption setYesorNo={setYesorNo} yesorNo={yesorNo} deleteInterruption={deleteInterruption}/> : <form className="calendar_div" /*onSubmit={handleSubmit}*/>
        <p className="holiday_title">İşçinin kəsinti tarixçəsi</p>
       ...
       {come.map( d => {  
        ...
        <Link className="reject" onClick={() =>  setOpenInterruption(false)} >Geri qayıt</Link>
        <button  className="add" onClick={() =>  setOpenInterruption(false)} type="submit">Yadda saxla</button>
      </form>}
    </FullScreen>
  )
}
export default EditInterruption;
