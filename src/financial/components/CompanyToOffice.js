...
function CompanyToOffice({ show , successTransfer, officeDataApi}) {
    ...
    useEffect(() =>{
      ...
      useEffect(() =>{
        if(openApi === 0){
          const url = `https://dev.kodaze.com/api/v1/transfer/company-transfer/?limit=${limitoffset.offset * (limitoffset.page + 1)}`;
          axios.get(url, {
            headers: {
          ...
          const url = `https://dev.kodaze.com/api/v1/transfer/company-transfer/?limit=${limitoffset.offset * (limitoffset.page + 1)}&sending_office=${totalInfo.send}&receiving_office=${totalInfo.make}&transfer_note=${totalInfo.comm}&transfer_date__gte=${totalInfo.sdate}&transfer_date__lte=${totalInfo.edate}`;
          axios.get(url, {
            headers: {
              "Content-Type": "application/json",
              "Authorization": `Bearer ${localStorage.getItem("token")}`,
        ...
        <FinancilalMainDiv>
                <FinancilalDiv className={show ? "small" : "big"}>
                    ...
                    <TopDiv>
                        ...
                        <p className="page_name">Maliyyə və Kredit/Şirkət ofis arası transfer</p>
                    </TopDiv>
                    <BlueLine>
                    ...
                    <Link to="/financial_credit/company_to_office/create"><Transfer>Transfer et</Transfer></Link>
...
export default CompanyToOffice;
