...
function CreatefromOffice
  ...
  console.log(making, sender, company);
  ...
  const handleSubmit = e => {
    ...
    const cony = 'https://dev.kodaze.com/api/v1/transfer/office-transfer/';
    return axios.post(cony, data, {
      ...
      .then(
        navigate("/financial_credit/office_to_office"),
  ...
  return (
    <FullScreenHolding className="h-full">
      <form className="calendar_div">
        ...
        <div className="datapicker_div">
          <div className="smalldiv">
            <select value={company} onChange={e => setCompany(e.target.value)} className="form_input" type="number">
              ...
              {
                companyData.map(x => (<option value={x.id}>{x.name}</option>))
              }
            </select>
          ...
          <div className="smalldiv">
            ...
            <p className="form_title">Göndərən ofis*</p>
          </div>
          <div className="smalldiv">
            <select value={making} onChange={e => setMaking(e.target.value)} className="form_input" type="number">
              <option disabled ></option>
              {officeData.map(x => {
                return x.company.id === parseInt(company, 10) ? <option value={x.id}>{x.name}</option> : ""
              })}
            </select>
            <p className="form_title">Qəbul edən ofis*</p>
          </div>
          <div className="smalldiv">
        ...
        <Link className="reject" to={"/financial_credit/office_to_office"}>Ləğv et</Link>
        <button className="add" onClick={handleSubmit} disabled={company === "" ? true : false}>Əlavə et</button>
      </form>
    </FullScreenHolding>
  )
}
export default CreatefromOffice
  ;